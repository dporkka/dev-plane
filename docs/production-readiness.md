...
- PR factory no longer creates fake local PR records when GitHub is unavailable. It requires a configured GitHub gateway/token, publishes the workspace branch through the shared VCS authority, calls the GitHub PR API, and only persists a PR record after GitHub returns a PR.
- Runtime-backed PR publication uses a separate privileged `VCSWorkspacePublisher` capability instead of agent `ExecuteCommand`: Docker workspaces remain `--network none`, reviewed state is staged outside the sandbox, credentials stay in the control-plane environment/request, Git hooks and credential helpers are disabled for the privileged push, and an explicit trusted GitHub remote URL is used instead of agent-controlled `origin`.
- Local and Docker workspace creation now route clone/worktree semantics through `packages/vcs`; isolated runtime destruction occurs before the workspace row is tombstoned so cleanup failure remains retryable.
...
- [ ] Re-run live PR creation with a Docker-backed workspace and, separately, through `RemoteProvider`; verify the agent container has no network, the approved branch reaches the expected GitHub repository, and no publication credential appears in agent command arguments or logs.
- [ ] Restore trustworthy hosted CI execution (tracked by issue #17); current GitHub Actions jobs are failing before any workflow step is assigned, so branch CI cannot presently provide repository validation evidence.
