...
import { CircleDot, FileDiff, FilePlus, Search, X } from "lucide-react";
...
export function FileBrowser({
        ...
        .map((node) => {
          if (node.type === "directory") {
            const filteredChildren = node.children
              ? filterNodes(node.children as FileWithStatus[])
              : [];
  ...
  const _handleContextMenu = (e: React.MouseEvent, node: FileWithStatus) => {
    e.preventDefault();
